...
def classwise_dice(output, gt):
    ...
    selected = torch.tensor([(output == i).sum() for i in range(n_classes)]).float()  # TP+FP
    relevant = torch.tensor([(gt == i).sum() for i in range(n_classes)]).float()  # TP+FN

    # precision = (true_positives + epsilon) / (selected + epsilon) # TP / TP + FP
    # recall = (true_positives + epsilon) / (relevant + epsilon)   # TP / TP + FN

    classwise_dice = (2 * true_positives + epsilon)/ (selected + relevant + epsilon)

    return classwise_dice
...
def classwise_precision(output, gt):
    """
    Args:
        output: torch.Tensor of shape (n_batch, n_classes, image.shape)
        gt: torch.LongTensor of shape (n_batch, image.shape)
...
def classwise_recall(output, gt):
    """
    Args:
        output: torch.Tensor of shape (n_batch, n_classes, image.shape)
        gt: torch.LongTensor of shape (n_batch, image.shape)
...
dice = make_weighted_metric(classwise_dice)
recall = make_weighted_metric(classwise_recall)
precision = make_weighted_metric(classwise_precision)


if __name__ == '__main__':
    output, gt = torch.zeros(3, 2, 5, 5), torch.zeros(3, 5, 5).long()
    print(classwise_recall(output, gt))
